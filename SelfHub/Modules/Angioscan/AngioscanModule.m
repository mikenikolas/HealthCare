...
#import "AngioscanModule.h"

@interface AngioscanModule ()

@end

@implementation AngioscanModule

@synthesize delegate, navBar, hostView, moduleView, slidingImageView, slidingMenu;
@synthesize rightSlideBarTable, backImView;
...
- (void)viewDidLoad
{
    ...
    lastSelectedIndexPath = [[NSIndexPath indexPathForRow:2 inSection:1] retain];
...
- (UIImage *)getModuleIcon{
    return [UIImage imageNamed:@"Fake_Moduls_Icons_angio.png"];
};
...
// Loading module data.
- (void)loadModuleData{
    NSString *listFilePath = [[self getBaseDir] stringByAppendingPathComponent:@"angioscan.dat"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if(section==1 || section==2){
        ...
        headerLabel.text = (section==1)? NSLocalizedString(@"Statistics", @"") : @"";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([indexPath section]==0){
        switch([indexPath row]){
            case 0:
                cellName.text = NSLocalizedString(@"NewTest", @"");
        ...
        switch([indexPath row]){
            case 0:
                cellName.text = NSLocalizedString(@"Report", @"");
                cell.moduleName.text= @"";
                break;
            case 1:
                cellName.text = NSLocalizedString(@"Pulse", @"");
                cell.moduleName.text= @"";
                break;
            case 2:
                cellName.text = NSLocalizedString(@"AgeOfVessels", @"");
                cell.moduleName.text= @"";
                break;
            case 3:
                cellName.text = NSLocalizedString(@"WaveMode", @"");
                cell.moduleName.text= @"";
                break;
            case 4:
                cellName.text = NSLocalizedString(@"Oxygen", @"");
                cell.moduleName.text= @"";
                break;
            case 5:
                cellName.text = NSLocalizedString(@"StressLevel", @"");
                cell.moduleName.text= @"";
                break;
            case 6:
                cellName.text = NSLocalizedString(@"Control", @"");
                cell.moduleName.text= @"";
                break;
        ...
        switch([indexPath row]){
            case 0:
                cellName.text = NSLocalizedString(@"Settings", @"");
                cell.moduleName.text= @"";
                break;
            case 1:
                cellName.text = NSLocalizedString(@"Help", @"");
                cell.moduleName.text= @"";
                break;
            default:
                break;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([indexPath section]==0){
        if([indexPath row]==0){
            if (screenBounds.size.height != 568) {
                [backImView setImage: [UIImage imageNamed:@"Angiascan_iphone_startTest.png"]];
            }else{
                [backImView setImage: [UIImage imageNamed:@"Angiascan_iphone_statistic_age—2—B_03_i5.png"]];
            }
        }
    } else if([indexPath section]==1){
        if([indexPath row]==2){
            if (screenBounds.size.height != 568) {
                [backImView setImage: [UIImage imageNamed:@"Angiascan_iphone_statistic.png"]];
            }else{
                [backImView setImage: [UIImage imageNamed:@"Angiascan_iphone_startTest_03_i5.png"]];
            }
        }
    };
};
